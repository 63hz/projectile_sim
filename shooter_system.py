...
5. Ball velocity = surface_speed * sqrt(efficiency)
...
class WideRollerShooter:
    ...
    def ball_exit_velocity_fps(self, roller_rpm: float) -> float:
        """Ball exit velocity accounting for efficiency."""
        # Ball velocity ≈ surface_speed * sqrt(efficiency)
        return self.surface_velocity_fps(roller_rpm) * np.sqrt(self.energy_transfer_efficiency)

    def rpm_for_ball_velocity(self, v_ball_fps: float) -> float:
        """Required roller RPM to achieve desired ball exit velocity."""
        # v_ball = v_surface * sqrt(eff)
        # v_surface = v_ball / sqrt(eff)
...
def calculate_optimal_gear_ratio(motor_type: str, target_velocity_fps: float,
    ...
    v_surface = target_velocity_fps / np.sqrt(0.8) * FT_S_TO_IN_S * IN_TO_M
